...
public:
	...
	~CzxTimer() {
		...
		// 打印时间差
		//log << "执行" << method << "经过的时间: " << ends - start << " 毫秒" << std::endl;
...
public:
	...
	static unordered_map<string, string> readProfile(string path)
	{
...
{
public:
	CzxComparison(CP cloud, int size1=1, int size2=1):before(new CloudT), after(new CloudT), s1(size1), s2(size2)
	{
		*before = *cloud;
		after = cloud;
...
namespace arsenal {
	void sortByAngle(CP cloud_in, vector<PointT> centerPoints= vector<PointT>(), int empty_dim=1);
	float computeAngleBetweenPoints2D(const pcl::PointXYZ& pointA, const pcl::PointXYZ& pointB, const pcl::PointXYZ& pointC, int empty_dim=1);
	vector<CP> splitPolygon(CP cloud_in, double angle_threshold=15, double dis_threshold=0.1);
}
