...
bool CubicBSplineCurve::verify(int index)
{
	int direction = ((controls_[index + 1][0] - controls_[index][0]) > 0) ? 1 : -1;
	for (int i = 1; i < index; i++)
	{
		float direction_cur = ((controls_[index + i + 1][0] - controls_[index + i][0]) > 0) ? 1 : -1;
		if (direction_cur != direction) return false;

	}
	return true;
}

void CubicBSplineCurve::setNewControl( const vector<Vector2d>& controlPs, bool veri)
{
	clear();
	controls_ = controlPs;
...
Vector2d CubicBSplineCurve::getPos( const Parameter& para) const
{
	...
	for( int i = 0; i < 4; i++)
	{
		//if (ki == 0)
		//{
		//	pm(i, 0) = controls_[0].x() / 6.0;
		//	pm(i, 1) = controls_[0].y() / 6.0;
		//}
		//else if ((ki + 4) >= n)
		//{
		//	pm(i, 0) = controls_[n-1].x() / 6.0;
		//	pm(i, 1) = controls_[n-1].y() / 6.0;
		//}
		//else
		//{
